...
def get_campaign_statistics_metrics(campaign: Campaign, session: Session) -> tuple[int, int, float, float]:
    try:
        statistics = (
            session.query(
            ...
            .filter(ClientToCampaign.campaign_id == campaign.id)
            .one()
        )
        return (
...
def get_daily_campaign_metrics(campaign: Campaign, session: Session) -> list[tuple[int, int, int, float, float]]:
