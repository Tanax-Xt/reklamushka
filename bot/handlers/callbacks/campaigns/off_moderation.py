...
@bot.callback_query_handler(func=lambda call: call.data == "off_moderation")
def off_moderation(call):
    bot.answer_callback_query(call.id)

    status_code, response = post_moderation_mode(False)

    if status_code == 200:
        bot.send_message(
            call.message.chat.id,
            moderation_mode_switch_success.format(mode="выключена"),
            parse_mode="Markdown",
        )
    else:
        bot.send_message(
